...
int Knn(int a,int b,double *values,double *test)
{
	...
	for (int i = 0; i < a; ++i)
		for (int j = 0; j < b; ++j) ans2.push_back(ans[i][j] + ans[i+1][j]);
	for (int i = 0; i < ans2.size()/a; ++i) ans3.push_back(sqrt(ans2[i])); 
	...
	cout << "--------------------------------------" << endl;
	for (int i = 0; i < ans2.size() / a; ++i) cout << sqrt(ans2[i]) << "\t   \t" << cls[i] << endl;
	cout << endl << "K = 3" << "\t" << "Class" << endl;
	cout << "------------------------------" << endl;
	for (int i = 0; i < 3; ++i) cout << ans3[i] << "\t  " << clss[i] << endl;
	int y=0,n=0;
	for (int i = 0; i < 3; ++i)
	{
		if(clss[i]==1) ++y;
